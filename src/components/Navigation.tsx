...
const Navigation = () => {
  ...
  const dropdownWidth = Math.max(longestItemName.length * 8 + 48, 140); // 8px per char + padding
      ...
      <div className="max-w-7xl mx-auto px-[5px]">
        <div className="flex justify-between items-center h-14 px-[20px] py-0">
          ...
          <div className="lg:hidden relative">
            ...
            {/* Mobile Dropdown - positioned absolutely to align with right edge */}
            {isOpen && (
              <div 
                className="absolute top-full right-0 mt-1 bg-white/20 backdrop-blur-xl border border-white/30 shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-300 ease-out animate-in slide-in-from-top-2"
                ...
                <div className="py-2">
                  {navItems.map((item) => (
                    <Link
                      key={item.name}
                      to={item.path}
                      className={`block px-4 py-2 font-medium transition-colors hover:text-yellow-600 hover:bg-white/10 text-center ${
                        isActive(item.path) ? "text-yellow-600 bg-white/5" : "text-gray-700"
                      }`}
